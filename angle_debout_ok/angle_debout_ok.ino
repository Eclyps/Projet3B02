...
void loop() {
  ...
  // -10 -0 pour roll c'est jambe droite tendu apres en pliant c'est val positive
  // rotation interne négative (pitch) roation externe négative (pitch)
...
void calculsVitesseDistance(float deltaTime, float& vitesse, float& distance, float& vitesseMax, float& distanceSprint) {
  ...
  float seuilAcc = 0.25;  // Seuil d'accélération (ajustez selon votre application)
  ...
  if (accelMagnitude < seuilAcc) {
    ...
    // Mise à jour de la vitesse avec les composants de l'accélération
    vitesse += filtax * deltaTime;  // Accélération en X
    vitesse += filtay * deltaTime;  // Accélération en Y
    vitesse += filtaz * deltaTime;  // Accélération en Z
  }

  // Limiter la vitesse à une valeur maximale réaliste (par exemple, 10 m/s)
  ...
  distance += vitesse * deltaTime;  // Distance = vitesse * temps écoulé
...
void postMessage(int postId) {
  HTTPClient http;
  String url = String(serverAddress) + "/post/message/" + String(postId) + "/Connected";
  http.begin(url);
