...
void loop() {
  ...
  pitch_diff = euler1.angle.pitch - euler2.angle.pitch;  //rot
  roll_diff = euler1.angle.roll - euler2.angle.roll;     // flex
...
void calculsVitesseDistance(float deltaTime, float& vitesse, float& distance, float& vitesseMax, float& distanceSprint) {
  ...
  float seuilAcc = 0.7;  // Seuil d'accélération (ajustez selon votre application)
